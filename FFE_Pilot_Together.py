...
nSubject = SubNumber('subNumTogether.txt')
...
dataFile.write('Participant_Number,Trial_Number,Condition,FC,Target_Contrast,Correct_Response,Reaction_Time\n')
...
conditions=[
    {"label":"Together",'startVal': .1, "FC":0.1, "nReversals":1, "stepType":'log', "stepSizes":0.1, "nUp":nup, "nDown":ndown, "nTrials":nReal*nBlocks, "minVal":0, "maxVal":.1},
...
#Null conditions. Are identical, except some (or all) lines do not get drawn
    ...
    #C_null shapes
    {"label":"Together_null",'startVal': .1, "FC":0.1, "nReversals":1, "stepType":'log', "stepSizes":0.1, "nUp":nup, "nDown":ndown, "nTrials":nReal*nBlocks, "minVal":0, "maxVal":.1},
...
condition_counters = [
    # Exp
    [{"label":"Together",'startVal': .1, "FC":0.1, "nReversals":1, "stepType":'log', "stepSizes":0.1, "nUp":nup, "nDown":ndown, "nTrials":nReal*nBlocks, "minVal":0, "maxVal":.1}, 1, nReal*nBlocks],
    # Null
    # C
    [{"label":"Together_null",'startVal': .1, "FC":0.1, "nReversals":1, "stepType":'log', "stepSizes":0.1, "nUp":nup, "nDown":ndown, "nTrials":nReal*nBlocks, "minVal":0, "maxVal":.1}, 1, nNull*nBlocks],
]
...
fixation_t = .3 #was .2 #pre-stim fixation period duration
ISI_t = .3
stim_t = .3 # stimulus duration (0.2)
...
#        tracker.sendMessage(f"TRIAL_START {trialNum} | {thisCondition['label']} | {thisIntensity}")
        ...
        line3.contrast = thisCondition['FC']
        line5.contrast = thisCondition['FC']
        ...
        if thisCondition['label'].startswith('Together'):
            OF.drawOrder([line3,line4,line5],mywin)
            core.wait(stim_t)
            OF.drawOrder(message2,mywin)
            trialClock.reset()
            rt = None
            thisResp = None
        elif thisCondition['label'].startswith('Seperate'):
            OF.drawOrder([line3,line5],mywin)
            core.wait(ISI_t)
            OF.drawOrder(line4,mywin)
            core.wait(stim_t)
            OF.drawOrder(message2,mywin)
            trialClock.reset()
            rt = None
...
#        tracker.sendMessage(f"TRIAL_END {trialNum} | {thisCondition['label']} | {thisIntensity}")
        ...
        dataFile.write(f"{nSubject},{trialNum},{thisCondition['label']},{thisCondition['FC']},{thisIntensity},{thisResp},{rt}\n")
